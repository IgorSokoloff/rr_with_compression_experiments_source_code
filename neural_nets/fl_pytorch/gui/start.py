...
import sys, os, platform, time, shutil, psutil, pickle, threading, math, socket
import datetime
...
from PyQt5.QtWidgets import QApplication, QMainWindow, QPushButton, QMessageBox, QDialog, QFileDialog
from PyQt5 import QtCore, QtGui, QtWidgets

QtWidgets.QApplication.setAttribute(QtCore.Qt.AA_EnableHighDpiScaling, True)
QtWidgets.QApplication.setAttribute(QtCore.Qt.AA_UseHighDpiPixmaps, True)

from generated import MainView, AboutBox, ConfigWidget, LogWindow, SimulationWidget, AnalysisWidget, MultiMachineSelector

from utils import gpu_utils, comm_socket, git_utils, algorithms
import utils, run, data_preprocess
...
#======================================================================================================================
compressorTypes = ["ident", "randk", "bernulli", "natural", 
                   "qsgd", "nat.dithering", "std.dithering", "topk", 
                   "rank_k", "terngrad"]

compressorFormats = ["", "randk:<percentage-of-D [0,100]>% | randk:k[1,D]", "bernulli:p[0,1]", "", 
                     "qsgd:levels", "nat.dithering:levels:norm",  "std.dithering:levels:norm", "topk:<percentage-of-D [0,100]>% | topk:<K>[1,D]", 
                     "rank_k:<percentage-of-D [0,100]>% | rank_k:<K>[1,D]", ""]
#======================================================================================================================
requestUpdateResultTable = False

def showArtificalDatasetParameters(show = True):
...
def onShowAboutDialog():
    ...
    programInfo += f"OS name: {system}/{release}/{version}\n"
    programInfo += f"Host: {socket.gethostname()} / IP: {socket.gethostbyname(socket.gethostname())} \n"
    programInfo += "\n"
    programInfo += f"Python interpretator: {sys.executable}\n"
...
def onMemoryInfo():
    ...
    memory_start = (process.memory_info().rss + process.memory_info().vms)  # in bytes
    ...
    for i in range(len(gpus_properties)):
        ...
        uiLogInfo("  {0}. Used {1:.2f} MBytes. Available {2:.2f} MBytes GDDR".format(gpus_properties[i].name, memory_gpu/1024.0/1024.0, gpus_properties[i].total_memory / (1024.0 ** 2)))
...
def onLogSystemInformation():
    ...
    uiLogInfo(f"  Path to python interpretator: {sys.executable}")
...
def isSimulationNeedEarlyStop(H):
    """Predicator which provide informatin do we need early stopping"""
...
#            send_scalars_to_master.append(client_stats['send_scalars_to_master'])
    ...
    #if len(dataload_duration) > 0: simulations_stats[job_id]["dataload_duration_avg"] =  sum(dataload_duration)/len(dataload_duration)
    #if len(inference_duration) > 0: simulations_stats[job_id]["inference_duration_avg"] =  sum(inference_duration)/len(inference_duration)
    #if len(backprop_duration) > 0: simulations_stats[job_id]["backprop_duration_avg"] =  sum(backprop_duration)/len(backprop_duration)
    #if len(full_gradient_oracles) > 0: simulations_stats[job_id]["full_gradient_oracles_avg"] =  sum(full_gradient_oracles)/len(full_gradient_oracles)
    #if len(samples_gradient_oracles) > 0: simulations_stats[job_id]["samples_gradient_oracles_avg"] =  sum(samples_gradient_oracles)/len(samples_gradient_oracles)
    #if len(send_scalars_to_master) > 0: simulations_stats[job_id]["send_scalars_to_master"] =  sum(send_scalars_to_master)/len(send_scalars_to_master)
...
def onUpdateStatusForExternalResources():
    ...
    for r in range(machinesView.tblMachines.rowCount()):
        ...
        if "cpu:-1" in descr["devices"]: it_use_cpu.setFlags(QtCore.Qt.ItemIsSelectable | QtCore.Qt.ItemIsEnabled | QtCore.Qt.ItemIsUserCheckable)
        else:                            it_use_cpu.setFlags(QtCore.Qt.NoItemFlags)

        if "gpu:0" in descr["devices"]: it_use_gpu1.setFlags(QtCore.Qt.ItemIsSelectable | QtCore.Qt.ItemIsEnabled | QtCore.Qt.ItemIsUserCheckable)
        else:                            it_use_gpu1.setFlags(QtCore.Qt.NoItemFlags)

        if "gpu:1" in descr["devices"]: it_use_gpu2.setFlags(QtCore.Qt.ItemIsSelectable | QtCore.Qt.ItemIsEnabled | QtCore.Qt.ItemIsUserCheckable)
        else:                            it_use_gpu2.setFlags(QtCore.Qt.NoItemFlags)

        if "gpu:2" in descr["devices"]: it_use_gpu3.setFlags(QtCore.Qt.ItemIsSelectable | QtCore.Qt.ItemIsEnabled | QtCore.Qt.ItemIsUserCheckable)
        else:                            it_use_gpu3.setFlags(QtCore.Qt.NoItemFlags)

        if "gpu:3" in descr["devices"]: it_use_gpu4.setFlags(QtCore.Qt.ItemIsSelectable | QtCore.Qt.ItemIsEnabled | QtCore.Qt.ItemIsUserCheckable)
        else:                            it_use_gpu4.setFlags(QtCore.Qt.NoItemFlags)
    #=================================================================================================================
...
def onTimerEvent():
    ...
    for i in range(len(simulations_by_rows)):
        ...
        # For comfortable of GUI user represent single device as single strins
        ...
        if simulations_stats[job_id]["finished"] and simulations_stats[job_id]["completed_rounds"] < simulations_stats[job_id]["rounds"]:
...
def cbxClientSamlingTypeSelection(index):
...
def currentCommandLineFromGUI():
    ...
    if configView.cbxDecayingStepSizeInvToEpoch.checkState() == Qt.Checked:
        cmdline.append("--decaying-lr-one-inv-epoch")
...
def onSelectDataset(index):
    ...
    if dataset == "generated_for_quadratic_minimization":
        ...
        showArtificalDatasetParameters(True)
    else:
        try:
        ...
        showArtificalDatasetParameters(False)
...
def onLoadExperimentResults():
    ...
    for fileName in fileNames:
        ...
        if obj is None:
            dlg = QMessageBox()
            dlg.setWindowTitle("Error")
            dlg.setText(f"Can not load result from '{fileName}'")
            dlg.setStandardButtons(QMessageBox.Ok)
            dlg.setIcon(QMessageBox.Critical)
            button = dlg.exec_()
            return
    ...
    uiLogInfo(f"#{len(list_of_loaded_job_ids)} simulations {list_of_loaded_job_ids} have been succcessfully loaded from: {fileNames}")
...
def onSaveExperimentResults(selected):
    ...
    uiLogInfo(f"#{len(simulations)} simulations {simulations} have been succcessfully serialized into: {fileName}")
    dlg = QMessageBox()
    dlg.setWindowTitle("Confirmation")
    dlg.setText(f"#{len(simulations)} simulations \n {simulations} have been succcessfully serialized into:\n {fileName}")
    dlg.setStandardButtons(QMessageBox.Ok)
    dlg.setIcon(QMessageBox.Information)
    button = dlg.exec_()
...
def includeToLegend():
    ...
    add_to_legend = ""
    if   selText == "job id":             add_to_legend = "{job_id}"
    elif selText == "compressors schema": add_to_legend = "{client_compressor}"
    elif selText == "algorithm name":     add_to_legend = "{algorithm}"
    elif selText == "algorithm options":  add_to_legend = "{algorithm_options}"
    elif selText == "number of local iterations":       add_to_legend = "{local_iterations}"
    elif selText == "global step size":   add_to_legend = "{global_lr}"
    elif selText == "local step size":    add_to_legend = "{local_lr}"
    elif selText == "comment":            add_to_legend = "{comment}"
    elif selText == "group name":         add_to_legend = "{group_name}"
    elif selText == "total rounds":       add_to_legend = "{rounds}"
    elif selText == "completed rounds":   add_to_legend = "{completed_rounds}"
    elif selText == "hostname":           add_to_legend = "{hostname}"
    elif selText == "dimension of x":     add_to_legend = "{D}"
    elif selText == "total number of clients":   add_to_legend = "{total_clients}"
    elif selText == "start time for simulation": add_to_legend = "{start_time}"
    elif selText == "aggregated experiments":    add_to_legend = "{aggregated}"
...
def onPlotResults(export_legend_to_file):
    ...
    for g, job_id in enumerate(job_ids):
        ...
        if 'total_samples' in H:
            for i in range(rounds):
                epochs[i] = sample_gradient_oracles[i] / H['total_samples']
        ...
        for round in range(rounds):
            ...
            for axis_type, axis_values in ( (xAxisType, X), (yAxisType, Y) ):
                if axis_type == "Epoch":
                    axis_values[round] = epochs[round]
    ...
    if analysisView.cbxLegendLogScalingY.checkState() == Qt.Checked and doNotAggregate:
        for p in plots:
            ...
            used_color = color[rasterizedPlots % len(color)]
            used_marker = p["marker"]

            if p["job_id"] == "QSGD":
                used_color = (0 / 255.0, 0 / 255.0, 255 / 255.0)
                used_marker = "o"

            if p["job_id"] == "QSGD-RR":
                used_color = (255 / 255.0, 165 / 255.0, 0 / 255.0)
                used_marker = "P"

            if p["job_id"] == "DIANA":
                used_color = (0 / 255.0, 255 / 255.0, 255 / 255.0)
                used_marker = "v"

            if p["job_id"] == "DIANA-RR":
                used_color = (255 / 255.0, 0 / 255.0, 0 / 255.0)
                used_marker = "*"
            ...
            ax.semilogy(p["X"], p["Y"], marker = used_marker, markevery = p["markevery"], linestyle = p["linestyle"], label = formatedLabel, color = used_color)
            rasterizedPlots += 1
    ...
    if analysisView.cbxLegendLogScalingY.checkState() != Qt.Checked and doNotAggregate:
        for p in plots:
            ...
            ax.plot(p["X"], p["Y"], marker = used_marker, markevery = p["markevery"], linestyle = p["linestyle"], label = formatedLabel, color = used_color)
            rasterizedPlots += 1
    ...
    if export_legend_to_file and legend != None:
        fig = legend.figure
        fig.canvas.draw()     # This garantees the placement of a a legend so legend.get_window_extent() will work correctly
...
def updateMemoryUsageStatus():
    ...
    cpu_load_fraction = cpu_acive_mbytes / (cpu_all_gbytes * 1024.0) * 100.0                     # Procent of used CPU DRAM memory by process

    totalSwapBytes, usedSwapBytes, freeSwapBytes, percentSwapUsage, _, _ = psutil.swap_memory()

    cpuMem = " CPU {0:.2f} MB /{1:.2f} GB[{2:.0f}%] / SYSTEM SWAP {3:.2f} MB/{4:.2f} GB [{5:.0f}%]".format(cpu_acive_mbytes, cpu_all_gbytes, cpu_load_fraction,
    ...
    for i in range(len(gpus_properties)):
        ...
        load_fraction = acive_mbytes / (all_gbytes * 1024.0) * 100.0           # Procent of used GPU DRAM memory by process
    ...
    if configView.cbxShowNumberOfWorks.checkState() == Qt.Checked:
        ...
        for i in range(max_workers):
            ...
            if work_items_for_thread_i > 0:
                cur_execute_tasks += 1                         # One task is executing right now
                queued_tasks += (work_items_for_thread_i - 1)  # The rest of the tasks are in the queue for a specific thread

        mainView.simulationJobsLabel.setText(f"Simulations - progress: {cur_execute_tasks}, max simultaneous: {max_workers}, in queue: {queued_tasks}")
    else:
...
def addMachineToMachineList():
    ...
    for i in range(len(address)):
        ...
        w.insertRow(w.rowCount())
        r = w.rowCount() - 1
        for c, item in enumerate((it_name, it_ip, it_port, it_gpus, it_use_cpu, it_use_gpu1, it_use_gpu2, it_use_gpu3, it_use_gpu4, it_online)):
            item.setFlags(item.flags() ^ QtCore.Qt.ItemIsEditable)
            w.setItem(r, c, item)
...
def spbParallelSimulationsValueChanged():
    ...
    if is_any_task_in_process == False:
        ...
        dlg.setText(f"Currently the simulation is in progress. Please adjust number of of parallel simulations after finish")
        dlg.setStandardButtons(QMessageBox.Ok)
        dlg.setIcon(QMessageBox.Information)
        button = dlg.exec_()           
...
def onTabWidgetInAnalysisChanged():
    ...
    analysisView.customPlotsStates[prev_index] = {"xaxe" : analysisView.cbxXaxis.currentIndex(), 
...
# For cutomize logic - you can override values from your UI file within your code, or if possible set in Qt Creator.
#======================================================================================================================
...
layouts_to_append_plots = [analysisView.loCustomPlot_1, analysisView.loCustomPlot_2, analysisView.loCustomPlot_3,
                           analysisView.loCustomPlot_4, analysisView.loCustomPlot_5, analysisView.loCustomPlot_6, analysisView.loCustomPlot_7]
...
analysisView.btnMoveSimulationUp.clicked.connect(moveSimulaionUp)
analysisView.btnMoveSimulationDown.clicked.connect(moveSimulaionDown)
...
logView.actionCmdLineGeneration.triggered.connect(lambda: onCmdLineGenerationForFinishedExperiments(withNewLines = True))
logView.actionCmdLineGeneration.triggered.connect(lambda: onCmdLineGenerationForCurrentExperiment(withNewLines = True))
logView.actionCmdLineSingleGeneration.triggered.connect(lambda: onCmdLineGenerationForFinishedExperiments(withNewLines = False))
logView.actionCmdLineSingleGeneration.triggered.connect(lambda: onCmdLineGenerationForCurrentExperiment(withNewLines = False))
...
for i in range(len(header) - 1): header.setSectionResizeMode(i, QtWidgets.QHeaderView.ResizeToContents)
header.setSectionResizeMode(len(header) - 1, QtWidgets.QHeaderView.Stretch)
...
simulationView.btnCancelAll.clicked.connect(onCanceAllSimulations)
...
configView.cbxClientCompressor.currentIndexChanged.connect(lambda index: configView.edtClientCompressorFormat.setText(compressorFormats[index]))
...
configView.btnGenerateInitSeed.clicked.connect(lambda: configView.edtRandomInitSeed.setText(str(gui_rnd.randint(1, 10**9))))
configView.btnGenerateRunSeed.clicked.connect(lambda: configView.edtRandomRunSeed.setText(str(gui_rnd.randint(1, 10**9))))

# Change sampling type handler
configView.cbxClientSamlingType.currentIndexChanged.connect(cbxClientSamlingTypeSelection)
configView.lblPoissonSampling.setVisible(False)
configView.edtPoissonSampling.setVisible(False)
...
utils.execution_context.is_simulation_need_ealystop_fn = isSimulationNeedEarlyStop
...
# Create a a possible selection for slected extrernal devices
